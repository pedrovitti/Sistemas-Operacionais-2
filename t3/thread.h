...
public:
    ...
    ~Thread() {
        ...
        if(_state == READY)
            _ready.remove(this);     //Retira da fila ready
    
        if(_state == SUSPENDED) 
	        _suspended.remove(this); //Retira da fila suspended
      
       /* Todas as threads que aguardavam (join) sao removida da fila waiting
          sao resumidas */
        if (!_waiting.empty()) {
           release_waiting();        
         }
